	...
	<img src="../logo/cool coder.png" style="max-height: 14%; max-width: 14%;margin-bottom: -4%;margin-top: -2%;"/>
  	<button id="toggle" class="button" style="max-width:2vw;max-height: 5vh; border:none;background-color: transparent;margin-left:78vw ;"><img src="../logo/lamp.png" style="max-width:5vw;max-height: 5vh;/">
        ...
        <table id="searchbar">
            <tr> 
               <?php if(isset($key)){?>
                    <td> <input type="text" name='key' placeholder="nama" value="<?php echo $key?>" > </td> 
                <?php }else{ ?>
                    <td> <input type="text" name='key' placeholder="nama"> </td>
                <?php } ?>
                <td> <input id="searchbox" type="submit" name='sdb'value="Cari"></td>
            </tr>
        </table>
   		 </form>
</table>
<?php if (isset($_GET['sdb'])){?>
	<?php 
	$key=$_GET['key'];
	$result = mysqli_query($connect, "SELECT * FROM member_web WHERE nama LIKE '%$key%'");
    $query = mysqli_query($connect, "SELECT * FROM member_web WHERE nama LIKE '%$key%'");
?>
	<table border="0.5vw" align="center" style="min-width:100%">
		...
		<?php while ($record = mysqli_fetch_assoc($result)) { ?>
        	<tr align="center">
            	...
            	<td><?=$record['no']?></td>
            	<td>
            	<a style="color: transparent;"href="../crud/delete.php?id=<?=$record['id']?>">
            		<button class="fas fa-archive"></i></button>
            	</a>
            	<a style="color: transparent;"href="../crud/edit.php?id=<?=$record['id']?>">
